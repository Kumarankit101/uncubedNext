...
export function BillingPricingClient({ plans }: { plans: Plan[] }) {
  ...
  return (
    <section id="pricing" className="py-32 px-6 relative">
      ...
      <div className="max-w-6xl mx-auto relative">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} viewport={{ once: true }} className="text-center mb-16">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }} viewport={{ once: true }} className="relative mb-20">
          ...
          <div className="relative backdrop-blur-md rounded-3xl p-12 bg-white/[0.03] border border-white/[0.08]">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2 }} viewport={{ once: true }} className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
              {plans.map((plan, index) => {
                const Icon = planIcons[index] || Zap;
                const currentPrice = plan.monthlyPrice;

                return (
                  <motion.div key={plan.id} initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3, delay: 0.3 + index * 0.05 }} viewport={{ once: true }} className="flex flex-col p-6 backdrop-blur-sm transition-all duration-300 group relative overflow-hidden rounded-[30px] hover:scale-105 bg-white/[0.05] border border-white/[0.12] hover:bg-white/[0.08] hover:border-white/[0.2] shadow-lg hover:shadow-white/10 hover:shadow-2xl hover:shadow-blue-500/20">
                    <div className="text-center mb-4">
                      ...
                      <div className="text-sm mb-4 text-gray-400">{plan.monthlyCredits} AI credits included</div>
                    </div>
